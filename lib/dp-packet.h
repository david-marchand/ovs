...
dp_packet_batch_add_array(struct dp_packet_batch *batch,
                          struct dp_packet *packets[], unsigned int n)
{
    unsigned int count = MIN(n, NETDEV_MAX_BURST - batch->count);

    if (count) {
        memcpy(&batch->packets[batch->count], packets, count * sizeof packets[0]);
        batch->count += count;
    }
