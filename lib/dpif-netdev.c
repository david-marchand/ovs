...
struct dp_netdev {
    ...
    /* Count of pmds currently reloading, the last one that finishes to reload
     * signals the main thread waiting for it */
...
pmd_info_show_rxq(struct ds *reply, struct dp_netdev_pmd_thread *pmd)
{
    if (pmd->core_id != NON_PMD_CORE_ID) {
        ...
        for (unsigned int i = 0; i < ARRAY_SIZE(pmd->reload_at); i++) {
            ...
            for (j = 0; j < pmd->upcall_count[i]; j++) {
                ds_put_format(reply, ",%"PRIu64, pmd->upcall_duration[i][j]);
            }
            for (; j < ARRAY_SIZE(pmd->upcall_duration[i]); j++) {
                ds_put_format(reply, ",");
            }
            ds_put_format(reply, ",");
            ds_put_format(reply, ",%"PRIu64,
                          pmd->start_polling_at[i] - pmd->reload_at[i]);
            ds_put_format(reply, ",");
            ds_put_format(reply, ",%"PRIu64,
                          pmd->end_polling_at[i] - pmd->start_polling_at[i]);
    ...
    OVS_REQUIRES(dp->port_mutex)
{
    ...
    uint64_t before_remove = rte_get_tsc_cycles();
    reload_affected_pmds(dp);
    uint64_t after_remove = rte_get_tsc_cycles();
    ...
    uint64_t before_add = rte_get_tsc_cycles();
    reload_affected_pmds(dp);
    uint64_t after_add = rte_get_tsc_cycles();

    VLOG_INFO("removing took %"PRIu64", adding took %"PRIu64", delta remove-add %"PRIu64"\n", after_remove - before_remove, after_add - before_add, before_add - before_remove);
...
dp_netdev_pmd_reload_done(struct dp_netdev_pmd_thread *pmd)
{
    ...
    if (atomic_count_dec(&pmd->dp->reloading_pmds) == 1) {
...
dp_netdev_del_pmd(struct dp_netdev *dp, struct dp_netdev_pmd_thread *pmd)
{
    ...
    if (pmd->core_id == NON_PMD_CORE_ID) {
        ...
        atomic_count_set(&dp->reloading_pmds, 1);
        dp_netdev_reload_pmd__(pmd);
        ovs_mutex_lock(&dp->cond_mutex);
        ovs_mutex_cond_wait(&dp->cond, &dp->cond_mutex);
        ovs_mutex_unlock(&dp->cond_mutex);
